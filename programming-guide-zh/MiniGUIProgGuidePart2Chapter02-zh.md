...
字体的渲染特效一般通过逻辑字体的风格指定。比如，在创建逻辑字体时，如果指定 `FS_WEIGHT_BOOK` 风格，则将使用低通滤波算法来处理矢量字体的字型边缘，在 MiniGUI 放大点阵字体的字型时，也会采用低通滤波算法来处理字型的边缘，从而消除因为自动放大而产生的边缘锯齿。__表 1.3__ 给出了 MiniGUI 支持的字体渲染特效。

__表 1.3__ MiniGUI 支持的字体渲染特效
