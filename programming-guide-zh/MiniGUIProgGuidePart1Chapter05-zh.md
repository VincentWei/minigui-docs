...
在 MiniGUI 中，控件的子类化实际是通过替换已有的窗口过程实现的。__清单 1.1__ 中的代码就通过控件类创建了两个子类化的编辑框，一个只能输入数字，而另一个只能输入字母：
...
这样，我们的时间编辑器就能正常工作了。该程序的部分代码见__清单 1.2__，完整源代码可见本指南示例程序包 `mg-samples` 中的 timeeditor.c 文件。__图 1.1__ 给出了时间编辑器的运行效果。
