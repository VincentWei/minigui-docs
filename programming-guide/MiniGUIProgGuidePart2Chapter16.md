...
*
* \return `mInvsbComp` * - the new created Invisible Component pointer, `NULL`
or failed
*
...
*
* \return `mInvsbComp` * - the Invisible Component pointer if success, `NULL` 
or failed
*
...
%CODE{cpp}%
%INCLUDE{"%ATTACHURL%/timer.c.txt"
pattern="^.*?//START_DECLARE_TIMER(.*?)//END_DECLARE_TIMER.*"}% 
```
```
...
%CODE{cpp}%
%INCLUDE{"%ATTACHURL%/timer.c.txt"
pattern="^.*?//START_UPDATE_TIME(.*?)//END_UPDATE_TIME.*"}% 
